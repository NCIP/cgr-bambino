...
public class TraceCanvas extends Canvas implements Observer {
  ...
  TraceCanvas (String filename, int start_position, boolean rc,
	       Scrollbar s) {
    this.scrollbar = s;
  ...
  public void update (Graphics g) {
    // update using double-buffering to existing buffer
    //    System.out.println("-> update");  // debug
    Dimension d = getSize();
    int status = trace.status();
    if (offscreen == null) {
      paint(g);
    } else if (status == TraceFile.LOADED) {
      buffer_paint(offscreen.getGraphics(), d.width, d.height);
	...
	if (trace_bytes_loaded > 0) {
      ...
      //      Funk.Gr.centerText(g, message);
      Funk.Gr.centerText(this, message);
    }
  }

  public void paint (Graphics g) {
    // first-time paint; create offscreen buffer
    Dimension d = getSize();
    offscreen = createImage(d.width, d.height);
    resized = true;
    // resized: not technically correct; window might have just been obscured
    // and now needs to be repainted from scratch.  However, this is 
    // reasonable enough for our purposes in buffer_paint below.
    update(g);
  }
  ...
  public void apply_absolute_delta(int abs_delta) {
    if (SCROLL_BY_BASES && BASE_SCROLL_CENTERED_MODE) {
      ...
      scrollbar.setMaximum(trace.num_bases);
      // set temporary range (will be overwritten later)
      center_requested = resized = false;
      // disable pending center request

      int v = start_center_base + abs_delta;
      int max = scrollbar.getMaximum();
      //      System.err.println("SCB:" + start_center_base + " sb max:" + max);  // debug
      if (v > max) v = max;
